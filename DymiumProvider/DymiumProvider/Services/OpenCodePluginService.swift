...
/// Service responsible for creating and managing the OpenCode plugin for Dymium authentication
/// This plugin ensures that every API request reads the token fresh from auth.json,
/// solving the issue where OpenCode caches the SDK/apiKey at initialization time.
final class OpenCodePluginService {
    static let shared = OpenCodePluginService()
    
    /// The directory where our plugin will be installed
    private let pluginDir = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent(".local/share/dymium-opencode-plugin")
    
    /// Path to the auth.json file that contains our tokens
    ...
    /// The plugin URL to be used in opencode.json
    var pluginUrl: String {
        "file://\(pluginDir.path)"
    }
    
    /// Ensure the plugin is created and up-to-date
    func ensurePlugin() throws {
        ...
        // Write package.json
        let packageJson = createPackageJson()
        let packageJsonPath = pluginDir.appendingPathComponent("package.json")
        try packageJson.write(to: packageJsonPath, atomically: true, encoding: .utf8)
        
        // Write index.ts (the main plugin code)
        let indexTs = createIndexTs()
        let indexTsPath = pluginDir.appendingPathComponent("index.ts")
        try indexTs.write(to: indexTsPath, atomically: true, encoding: .utf8)
        
        print("✅ Dymium OpenCode plugin created at \(pluginDir.path)")
    }
    
    /// Create the package.json content
    private func createPackageJson() -> String {
        return """
        {
    ...
    /// Create the index.ts plugin code
    /// This plugin provides a custom fetch function that reads the token fresh from auth.json on every request
    private func createIndexTs() -> String {
        return """
        import fs from "fs"
